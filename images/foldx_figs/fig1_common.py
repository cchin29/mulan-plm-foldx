"""Figure 1 — shared main-architecture diagram for the four panels 1a–1d.

The top of every panel is the same MuLAN architecture (the visual abstract's left
panel, redrawn in the shared grammar). Each panel adds exactly one thing to it, and
expands that thing in a detail block below. Comparing panels side by side shows what
each route changed.

Panel letters follow deck order: 1a → 1b → 1c → 1d, then Figure 2, then Figure A1.

Conventions: solid arrow = data flow · dashed leader = "expanded below".
"""
from pathlib import Path as _Path

# `mulanfig` lives beside this module, so resolve from __file__ rather than from the working
# directory — the four panels import this one, and they are run from wherever is convenient.
...
SEQ_X,  SEQ_W  = 84,  228
PRE_X,  PRE_W  = 330, 104                     # reserved: 1c's AA+3Di fusion
ENC_X,  ENC_W  = 452, 104
GAP_X,  GAP_W  = 578, 150                     # reserved: 1a's late concat
ATT_X,  ATT_W  = 748, 128
SLAB_X         = 908
CMB_X          = 978
MINUS_X        = 1064
...
TABS = [("1a", "late concat"),
        ...
        ("1d", "score channel")]
TAB_SUB = {
    "1a": "Late concat of a WT-3Di block, between the embedder and Light Attention   ·   encoder: ProstT5",
    "1b": "A different hidden layer of the embedder   ·   probed on ProstT5 and Ankh-large, run in ProstT5",
    "1c": "Structure fused into the input token, and the attempt to make it mutation-specific   ·   encoder: SaProt-650M",
    "1d": "A physics ΔΔG into the head's add_scores slot   ·   encoder: Ankh-large",
}
TAB_OUT = {
    "1a": ("bad",  "hurts",             "0.740 → 0.663"),
    "1b": ("bad",  "hurts",             "0.648  ·  0.429"),
    "1c": ("good", "helps  /  no-op",   "0.842   ·   0 / 39"),
    "1d": ("warn", "real, but bounded", "+0.009  ·  +0.017"),
}


def header(active):
    o = [text(56, 48, "Figure 1  ·  Four routes for structure into MuLAN", 24, INK, anchor="start", weight=650),
    ...
    for tag, nm in TABS:
        ...
        o.append(text(x + 50, 113, nm, 11.5, "#fff" if on else MUTED, anchor="start"))
        x += w + 8
...
def arch(active, gap=None, pre=None, enc_mark=False, slot_mark=False, dim_others=False):
    ...
    columns; the *_mark flags highlight what this panel changes."""

"""Figure B — What each predictor is given at test time, and what that buys."""
...
o.append(text(56, 50, "Predictor inputs at test time — FoldX alone, MuLAN base, MuLAN + channel", 25, INK, anchor="start", weight=650))
o.append(text(56, 80,
...
o.append(result_block(Y1 + 92,
                      ...
                      [("lane 1  FoldX", 0.363, PHYS_S, 1),
                       ("lane 2  base", 0.142, "#8ea9c9", 2),
                       ("lane 3  + channel", 0.354, ATT_S, 3)],
                      "Lane 3 lands on lane 1. Δ = −0.008 [−0.035, +0.019]."))
...
o.append(result_block(Y2 + 138,
                      ...
                      [("lane 1  FoldX", 0.398, PHYS_S, 1),
                       ...
                       ("lane 3  + MLP", 0.525, ATT_S, 3)],
                      "Lane 3 clears lane 1. Δ = +0.127 [+0.048, +0.216]."))
...
o.append(text(PX + 24, Y3 + 188, "which happens on 18 of 226 contrasts.", 11, INK_SOFT, anchor="start"))
...
stats = [("0 of 79", "base arms that beat FoldX alone", BAD),
         ("18 of 226", "contrasts that significantly beat it — all carry the channel", GOOD),
         ("1 of 56", "wins on the clustered tiers, where the method actually fails", WARN)]
...
write(str(_OUT / "fig_b_what_each_predictor_sees.svg"), W, H, "".join(o))
print("ok")

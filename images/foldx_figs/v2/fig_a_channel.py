"""Figure A (appendix) — the FoldX channel reference sheet.

Tab Civ is the main slide: it shows *where* the arms attach. This sheet is the
...
o.append(text(56, 40, "APPENDIX  ·  REFERENCE", 11, MUTED, anchor="start", weight=700))
o.append(text(56, 66, "The FoldX channel — definitions and provenance", 24, INK, anchor="start", weight=650))
o.append(text(56, 92,
              "Everything behind tab Civ: how the number is computed, what the twelve terms are, and the exact two configurations. "
...
o.append(text(PX + 26, CY + 263, "Matched on (wt_aa, pos, mut_aa), duplicates disambiguated role→group, every mapping checked against the repaired PDB.",
              10.5, INK_SOFT, anchor="start"))
o.append(text(PX + 26, CY + 281, "Coverage — 87.7 % single-point, 98.7 % multi-point, 90.8 % combined. Uncovered rows fall back to the per-fold standardised mean (0).",
              10.5, MUTED, anchor="start"))
...
o.append(text(gx, SY + 236, "See tab Civ for the arms", 10.5, MUTED, anchor="start", style="font-style:italic"))
